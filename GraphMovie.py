...
class GraphMovie(object):
    ...
    def plotFrame(self, g, label, layout):
        ...
        p = igraph.drawing.plot(g, frameFile,layout = layout, bbox = (self.pixelsX, self.pixelsY), margin = self.margin)
        p.save(frameFile)
    ...
    def renderMovie(self):
        ...
        '''
        #make sure file empty 
        ALL = '*'
        cleaningString = 'rm %s'%os.path.join(os.getcwd(),self.frameDirectory,ALL)
        os.system(cleaningString)
        ...
        os.system(cmdString)
        convertString = 'ffmpeg -i ./output.avi -acodec libfaac -b:a 128k -vcodec mpeg4 -b:v 1200k -flags +aic+mv4 ./output.mp4'
